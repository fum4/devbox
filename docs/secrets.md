...
| `ansible/secrets/jarvis-age-key.age` | the **jarvis repo's** age private key | `repo-age-keys` role → `~/.config/age/jarvis.key` (mode 0600). Lets the box run `bin/jarvis-tf` + decrypt jarvis's secrets. | Yes (same) |
