...
- **All secrets, every repo, go through the age store** — never leave a secret existing *only* as gitignored plaintext in a repo's working tree. That's the same drift trap as a hand-edited live config: a devbox rebuild or laptop loss wipes it. Any project on the box that accumulates secrets (API tokens, deploy creds, SSH keys, `.env` values) enrolls them as `ansible/secrets/<repo>-<name>.age`, placed onto the box by a per-repo role that decrypts on the laptop (the `github-identity` / `expo-identity` pattern). The single root of trust is the age key in your password manager; provider logins + 2FA recovery codes live there too. Full procedure + the per-repo enrollment recipe: [`docs/secrets.md`](../docs/secrets.md). (kost's infra secrets are the first pending enrollment — see that doc.)
